...
def _parse_part_weighted_metric(metric, num_bin, device, dtype):
    """Parse a legacy metric string or a complete set of weighted part groups."""
    if isinstance(metric, str):
        return metric, None
    if not isinstance(metric, dict):
        raise TypeError("metric must be a string or a mapping")

    metric_type = metric.get("type")
    if metric_type not in ("euc", "cos"):
        raise ValueError("metric['type'] must be 'euc' or 'cos'")
    groups = metric.get("part_groups")
    if not isinstance(groups, list) or not groups:
        raise ValueError("metric['part_groups'] must be a non-empty list")

    weights = torch.zeros(num_bin, device=device, dtype=dtype)
    covered = torch.zeros(num_bin, device=device, dtype=torch.bool)
    for group in groups:
        if not isinstance(group, dict):
            raise TypeError("each part group must be a mapping")
        ...
        if (
            ...
            or not 0 <= start < end <= num_bin
        ):
            raise ValueError(
                f"invalid part range [{start}, {end}) for {num_bin} parts"
            )
        if (
            not isinstance(group_weight, (int, float))
            or group_weight <= 0
        ):
            raise ValueError("part-group weights must be positive")
        if covered[start:end].any():
            raise ValueError("part groups must not overlap")
        covered[start:end] = True
        weights[start:end] = group_weight / (end - start)

    if not covered.all():
        raise ValueError("part groups must cover every embedding part")
    expected = torch.tensor(1.0, device=device, dtype=dtype)
    if not torch.isclose(weights.sum(), expected, atol=1e-6):
        raise ValueError("part-group weights must sum to 1.0")
    return metric_type, weights
...
def cuda_dist(x, y, metric='euc'):
    ...
    num_bin = x.size(2)
    metric_type, part_weights = _parse_part_weighted_metric(
        metric, num_bin, x.device, x.dtype
    )
    if metric_type == 'cos':
    ...
    for i in range(num_bin):
        ...
        _y = y[:, :, i]
        if metric_type == 'cos':
            part_dist = torch.matmul(_x, _y.transpose(0, 1))
        else:
            _dist = torch.sum(_x ** 2, 1).unsqueeze(1) + torch.sum(_y ** 2, 1).unsqueeze(
                0) - 2 * torch.matmul(_x, _y.transpose(0, 1))
            part_dist = torch.sqrt(F.relu(_dist))
        dist += (
            part_dist
            if part_weights is None
            else part_weights[i] * part_dist
        )
    if part_weights is None:
        return (
            1 - dist / num_bin
    ...
    return 1 - dist if metric_type == 'cos' else dist
